...
logistic_reg() %>%
  #Set the type of regression (GLM is default)
  set_engine("glm") %>%
  #Set the actual parameters of the model that it's fitting
...
# Do a logistic regression -- on just the 1% training data
logistic_reg() %>%
  #Set the type of regression (GLM is default)
  set_engine("glm") %>%
  #Set the actual parameters of the model that it's fitting using our training data
  fit(dvote ~ gender + party + state + office, data = training) %>%
  #Predict using the model on the testing dataset
  predict(training) %>%
  bind_cols(training) %>%
  metrics(truth = dvote, estimate = .pred_class)%>%
  gt() %>% 
  tab_header("Model 4 Evaluation on All Data")

# Do a logistic regression -- using training data then testing it on testing data
logistic_reg() %>%
  #Set the type of regression (GLM is default)
  set_engine("glm") %>%
  #Set the actual parameters of the model that it's fitting using our training data
  ...
  metrics(truth = dvote, estimate = .pred_class)%>%
  gt() %>% 
  tab_header("Model 5 Evaluation on All Data")
